...
public class RotatedWithStackPoints extends JFrame implements GLEventListener{
    ...
    // define the stack
    private static MatrixStack mvStack = new MatrixStack(20);
    ...
    public RotatedWithStackPoints(){
        setTitle("Two points rotated with stack");
        setSize(600, 400);
        setLocation(200, 200);
    ...
    public void display(GLAutoDrawable drawable){
        ...
        // to be used as uniform variables in shaders
        int mv_location = gl.glGetUniformLocation(rendering_program, "mv_matrix");
        // we will put our rotation matrix in mv_location
        // then it will be read by "mv_matrix" in the shaders
        
        mvStack.pushMatrix(); // stack now has two (non-empty) levels
        double rotationDeg = (double) -45;
        mvStack.rotate(rotationDeg, 0, 0, 1);

        
        // now make it available to the "mv_matrix"
        ...
        // be careful: we have TWO points!
        gl.glDrawArrays(GL_POINTS, 0, 2);
        
        mvStack.popMatrix(); // this cleans the stack
        
        }
    
    public static void main(String[] args)
    { 
        new RotatedWithStackPoints();
    }
    ...
    public int createShaderProgram() {
        ...
        String vshaderSource[] = util.readShaderSource("vert_7a.shader");
        String fshaderSource[] = util.readShaderSource("frag_7a.shader");
