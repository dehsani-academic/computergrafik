...
public class SphereFloorDifferentMaterialsPtLight extends JFrame implements GLEventListener
{   
    ...
    public SphereFloorDifferentMaterialsPtLight()
    {   setTitle("Point light on sphere");
        setSize(600, 400);
        setLocation(200, 200);
    ...
    public void installLights(int rendering_program, Matrix3D viewMatrix) {
        ...
        Point3D lightLoc = new Point3D(2.0f, 2.0f, 2.0f); 
        ...
        pl.setDiffuse(dif); 
        pl.setSpecular(spec); 
        pl.setPosition(lightLoc); 

        // see the attenuation formula for a point source light
        pl.setConstantAtt(1.0f); // k_c 
        pl.setLinearAtt(0.001f); // k_l 
        pl.setQuadraticAtt(0.002f); // k_q 
        
        float[] attenuationArray = new float[] {
                pl.getConstantAtt(), pl.getLinearAtt(), pl.getQuadraticAtt()
        };

        // convert light’s position to view space, and save it in a float array
        Point3D lightP = pl.getPosition();
        ...
        gl.glUniform4fv(ambLoc, 1, pl.getAmbient(), 0);
        gl.glUniform4fv(diffLoc, 1, pl.getDiffuse(), 0);
        gl.glUniform4fv(specLoc, 1, pl.getSpecular(), 0);
    ...
    public void passOne() {
        ...
        Vector3D lightEye = new Vector3D(
                pl.getPosition().getX(), pl.getPosition().getY(), pl.getPosition().getZ());
        ...
        lightProj_matrix = perspective(70.0f, aspect, 0.1f, 1000.0f);  
    ...
    public static void main(String[] args) {
        new SphereFloorDifferentMaterialsPtLight(); 
    }
    ...
    {   
        ...
        String v1shaderSource[] = util.readShaderSource("vert1_16a.shader");
        String v2shaderSource[] = util.readShaderSource("vert2_16a.shader");
        String f2shaderSource[] = util.readShaderSource("frag2_16a.shader");
