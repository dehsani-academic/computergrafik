...
public class SphereSeveralLights extends JFrame implements GLEventListener
{   
    ...
    // make positional lights
    // for simplicity only position and colors will be considered
    private int numPtLights = 2;
    private float [] pLightsPos = new float [numPtLights * 4];
    private float [] pLightsCol = new float [numPtLights *4];;
    ...
    public SphereSeveralLights()
    {   setTitle("Several lights around sphere");
        setSize(600, 400);
        setLocation(200, 200);
    ...
    public void installLights(int rendering_program, Matrix3D viewMatrix) {
        ...
        gl.glUniform4fv(materialAmbLoc, 1, currentMaterial.getAmbient(), 0);
        gl.glUniform4fv(materialDiffLoc, 1, currentMaterial.getDiffuse(), 0);
        gl.glUniform4fv(materialSpecLoc, 1, currentMaterial.getSpecular(), 0);
        gl.glUniform1f(materialShineLoc, currentMaterial.getShininess());
        
        // now do positional lights 
        // pLight 1 at -2,1,0 with blue color
        Vector3D pl1PosVec = new Vector3D(-2,1,0,1);
        Vector3D plPosVectransform = pl1PosVec.mult(viewMatrix);
        pLightsPos[0] = (float) plPosVectransform.getX();
        pLightsPos[1] = (float) plPosVectransform.getY();
        pLightsPos[2] = (float) plPosVectransform.getZ();
        pLightsPos[3] = (float) plPosVectransform.getW();
        ...
        // pLight 2 at 2,1,0 with red color
        Vector3D pl2PosVec = new Vector3D(2,1,0,1);
        plPosVectransform = pl2PosVec.mult(viewMatrix);
        pLightsPos[4] = (float) plPosVectransform.getX();
        pLightsPos[5] = (float) plPosVectransform.getY();
        pLightsPos[6] = (float) plPosVectransform.getZ();
        pLightsPos[7] = (float) plPosVectransform.getW();
        ...
        // set attenuation of both to be same
        float[] attenuationArray = new float[] {
                1.0f, 0.05f, 1.0f
        };
        
        // associate the arrays to uniform variables in the shaders
        int ptPosLoc = gl.glGetUniformLocation(rendering_program, "pLightPos");
        int ptColLoc = gl.glGetUniformLocation(rendering_program, "pLightCol");
        int attLoc = gl.glGetUniformLocation(rendering_program, "pLightAtt");
        
        gl.glUniform4fv(ptPosLoc, numPtLights, pLightsPos, 0);
        gl.glUniform4fv(ptColLoc, numPtLights, pLightsCol, 0);
    ...
    public void passOne() {
        ...
        lightProj_matrix = perspective(0.47f, aspect, 0.1f, 1000.0f);  
    ...
    public void passTwo(){
        ...
        // now set the material to GOLD for the floor
    ...
    {   GL4 gl = (GL4) GLContext.getCurrentGL();
        ...
        // now the tiny spheres around the lights
        Sphere l1Sphere = new Sphere(96);
        Vertex3D[] l1vertices = l1Sphere.getVertices(); 
        int[] l1indices = l1Sphere.getIndices();
        
        float[] l1positions = new float[l1vertices.length*3];
        float[] l1normals = new float[l1vertices.length*3];
        
        Vector3D l1Translation = new Vector3D(-2,1,0);
        
        // scale the sphere
        for (int i=0; i < l1vertices.length; i++) {
            Point3D scaledPos = l1vertices[i].mult(0.2);
            l1positions[i*3] = (float) (scaledPos.getX() + l1Translation.getX());
            l1positions[i*3+1] = (float) (scaledPos.getY() + l1Translation.getY());
            l1positions[i*3+2] = (float) (scaledPos.getZ() + l1Translation.getZ());
            l1normals[i*3] = (float) l1vertices[i].getNormalX();
            l1normals[i*3+1]= (float)l1vertices[i].getNormalY();
            l1normals[i*3+2]=(float) l1vertices[i].getNormalZ();
        }
        
        // init l1 sphere
        initObject(2, l1positions, l1normals, l1indices, Types.TRIANGLES);
        
        // second light
        Sphere l2Sphere = new Sphere(96);
        Vertex3D[] l2vertices = l2Sphere.getVertices(); 
        int[] l2indices = l2Sphere.getIndices();
        
        float[] l2positions = new float[l2vertices.length*3];
        float[] l2normals = new float[l2vertices.length*3];
        
        Vector3D l2Translation = new Vector3D(2,1,0);
        
        // scale the sphere
        for (int i=0; i < l2vertices.length; i++) {
            Point3D scaledPos = l2vertices[i].mult(0.2);
            l2positions[i*3] = (float) (scaledPos.getX() + l2Translation.getX());
            l2positions[i*3+1] = (float) (scaledPos.getY() + l2Translation.getY());
            l2positions[i*3+2] = (float) (scaledPos.getZ() + l2Translation.getZ());
            l2normals[i*3] = (float) l2vertices[i].getNormalX();
            l2normals[i*3+1]= (float)l2vertices[i].getNormalY();
            l2normals[i*3+2]=(float) l2vertices[i].getNormalZ();
        }
        
        // init l2 sphere
        initObject(3, l2positions, l2normals, l2indices, Types.TRIANGLES);
