...
public class SquareWithIndices extends JFrame implements GLEventListener{
    ...
    {   GL4 gl = (GL4) GLContext.getCurrentGL();
        ...
        gl.glGenBuffers(vbo.length, vbo, 0); // generate the (three!) buffers

        gl.glBindBuffer(GL_ARRAY_BUFFER, vbo[0]); // bind the first buffer
        FloatBuffer squarePtsBuf = Buffers.newDirectFloatBuffer(square_verts);
        gl.glBufferData(GL_ARRAY_BUFFER, squarePtsBuf.limit()*4, squarePtsBuf, GL_STATIC_DRAW); // add data to buffer     
        
        // now do the same to collect the color data
        gl.glBindBuffer(GL_ARRAY_BUFFER, vbo[1]);
        FloatBuffer squareColorBuf = Buffers.newDirectFloatBuffer(square_colors);
        gl.glBufferData(GL_ARRAY_BUFFER, squareColorBuf.limit()*4, squareColorBuf, GL_STATIC_DRAW);   
